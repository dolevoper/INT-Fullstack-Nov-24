...
function mainMenu(input) {
    if (input === 1) {
        var tryNum = difficulty(Number(prompt("Starting new game! please choose diffiuclty(enter num of tries): easy - 5/ normal -3/ hard -1")));
        var randomN = range(Number(prompt("Choose range 1-100")));
        ...
        for (var i = 1; i <= tryNum; i++) {
            ...
            if (isNaN(newTry) || newTry > 100 || newTry === 0 || newTry === null) {
                ...
                alert("cold, the random number is " +
                    randomN);
            }
            else if (Math.abs(newTry - randomN) < 10) {
                alert("hotter, the random number is " +
                    randomN);
            }
            else {
                alert("you won!");
...
function GuessGameNew() {
    var firstMesssage = Number(prompt("Welcome to out Guess game!Choose (write): \n" +
