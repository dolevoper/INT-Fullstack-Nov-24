...
while (true) {
  const userInput = prompt(
    "Enter your choice: new game(1), change settings(2), or quit(3)"
  );
  switch (userInput) {
    case "1":
      let secretNumber = Math.floor(Math.random() * (10 * difficulty)) + 1;
      if (playMode === 1) {
        for (let i = 0; i < numberOfGuess; i++) {
          const guess = Number(
            prompt("Enter your guess(" + (i + 1) + "/" + numberOfGuess + ")")
          );
    ...
    case "2":
      ...
      playMode = Number(
        prompt("Enter play mode(1 for classic, 2 for hot/cold)")
      );
      break;
  }
  if (userInput === "3") {
