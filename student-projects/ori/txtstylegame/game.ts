...
function showInventory() {
    if (inventory.length === 0) {
        ...
        const inventoryList = inventory.map(item => `- ${item.name}: ${item.description}`).join("\n");
...
function forgottenTemple() {
    const room = roomStates.forgottenTemple;
    const userAction = prompt(
        `The crumbling walls of a long-abandoned temple rise before you. ${room.hasTakenKey ? "The pedestal where the key rested is now empty." : "A shiny key rests on a pedestal."}\n` +
...
function enchantedForrestClearing() {
    const room = roomStates.enchantedForrestClearing;
    const userAction = prompt(
        `A serene clearing surrounded by ancient oaks. ${room.hasTakenKey ? "The spot where the rusty key was buried is now empty." : "A rusty key is half-buried in the soft ground."}\n` +
...
function hiddenChamber() {
    const userAction = prompt(
        "A small, hidden chamber deep beneath the earth. An old chest lies in the corner, untouched for centuries.\n" +
